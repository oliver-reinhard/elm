...
public class ChatServlet extends HttpServlet {
	...
	// Class to hold message queue for each chat-room member
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// If a message is set, we only want to enter poll mode if the user is a new user. This is necessary to avoid two parallel requests per user (one is
		// already in async wait and the new one). Sending a message will dispatch to an existing poll request if necessary and the client will issue a new
		// request to receive the next message or long poll again.
